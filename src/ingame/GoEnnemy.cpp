...
GoEnnemy::GoEnnemy(const sf::Vector2f &position, GameManager &manager,
		   const std::string &ennemyName, const Stats &ennemy) noexcept
	: IGoEntity(position, ennemyName, ennemy)
...
{
	addComponent<CpnWandering>(manager, 2);
}
...
void GoEnnemy::start(sfs::Scene &scene) noexcept
{
	...
	Serializer s;
	if (texture == nullptr) {
		std::cerr << "Unable to find " << _spriteSheet << " asset" << std::endl;
		destroy();
		return;
	}
