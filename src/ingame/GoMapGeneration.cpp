...
: _mapSize(), _manager(gameManager)
{
    std::random_device rd;
    _gen = std::mt19937 (rd());
    _randMapSize = std::uniform_int_distribution<> (7, 10);
    _mapSize = _randMapSize(_gen);
...
void MapGenerator::start(sfs::Scene &scene) noexcept
{
    for (int i = 0; i < _mapSize + _mapSize % 2; i += 1) {
        ...
        goUp.setPosition(i * goUp.getGlobalBounds().width + goUp.getGlobalBounds().width / 2, 0);

        auto &goDown = addChild<GoWallHorizontal>(scene, scene, _manager);
        goDown.setPosition(i * goDown.getGlobalBounds().width,
         (_mapSize * goDown.getGlobalBounds().width) + (_mapSize % 2 * goDown.getGlobalBounds().width));
        ...
        goRight.setPosition( _mapSize * goRight.getGlobalBounds().height + (_mapSize % 2 * goRight.getGlobalBounds().height),
         i * goRight.getGlobalBounds().height + goRight.getGlobalBounds().height / 2);
